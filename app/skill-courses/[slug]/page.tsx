...
export default function SkillPage({ params }: SkillPageProps) {
  ...
  return <main>
    <section className="border-b bg-white"><div className="shell py-20 sm:py-28">
      ...
      <div className="mt-6 inline-flex items-center rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-bold text-cobalt">◷ {skill.duration}</div>
    ...
    <section className="section shell" id="objectives"><div className="grid gap-16 lg:grid-cols-[.75fr_1.25fr]">
      ...
      <div><p className="eyebrow">Learning objectives</p><div className="mt-7 divide-y border-y">{skill.learningOutcomes.map((item,index)=><div className="flex gap-5 py-6" key={item}><span className="font-display text-xl text-cobalt">{String(index+1).padStart(2,'0')}</span><p className="font-display text-2xl">{item}</p></div>)}</div><p className="eyebrow mt-14">Course coverage</p><div className="mt-7 divide-y border-y">{skill.syllabus.map((item,index)=><div className="grid grid-cols-[36px_1fr] gap-4 py-5" key={item}><span className="text-xs font-bold text-cobalt">{String(index+1).padStart(2,'0')}</span><p className="text-sm leading-6 text-zinc-600">{item}</p></div>)}</div>{next&&<a href={`/skill-courses/${nextSlug}`} className="mt-10 block rounded-2xl bg-night p-8 text-white"><p className="eyebrow !text-blue-300">Next recommended Skill Course</p><h2 className="mt-5 flex items-center gap-3 font-display text-4xl">{next.name} <ArrowRightIcon className="size-6"/></h2><p className="mt-3 text-sm text-zinc-400">{next.summary}</p></a>}</div>
    </div></section>
