...
export default function SkillPage({ params }) {
  ...
  return <main>
    <section className="border-b bg-white"><div className="shell py-20 sm:py-28">
      ...
      <div className="mt-8 flex flex-col gap-3 sm:flex-row"><button className="button-primary">Enroll in Course →</button><a href="#objectives" className="button-secondary">View objectives</a></div>
    </div></section>
    <section className="section shell" id="objectives"><div className="grid gap-16 lg:grid-cols-[.75fr_1.25fr]">
      ...
      <div><p className="eyebrow">Learning objectives</p><div className="mt-7 divide-y border-y">{['Understand the core concepts and professional context','Apply the skill through guided, realistic practice','Evaluate quality, risk, and tradeoffs','Connect this skill to the next roadmap milestone'].map((item,index)=><div className="flex gap-5 py-6" key={item}><span className="font-display text-xl text-cobalt">0{index+1}</span><p className="font-display text-2xl">{item}</p></div>)}</div>{next&&<a href={`/skill-courses/${nextSlug}`} className="mt-10 block rounded-2xl bg-night p-8 text-white"><p className="eyebrow !text-blue-300">Next recommended Skill Course</p><h2 className="mt-5 font-display text-4xl">{next[0]} →</h2><p className="mt-3 text-sm text-zinc-400">{next[1]}</p></a>}</div>
    </div></section>
