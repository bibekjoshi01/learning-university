...
export default function RolePage({params}: RolePageProps) {
  ...
  return <main>
    <section className="bg-night text-white"><div className="shell py-20 sm:py-28">
      ...
      <div className="mt-9 flex flex-col gap-3 sm:flex-row"><button className="button-primary">Enroll in Career Track</button><a className="button-secondary" href="#roadmap">View Roadmap</a></div>
    ...
    <section className="section shell" id="roadmap"><div className="grid gap-14 lg:grid-cols-[.65fr_1.35fr]">
      ...
      <div><p className="eyebrow">Learning roadmap</p><h2 className="mt-6 font-display text-5xl sm:text-6xl">Skills build in sequence.</h2><div className="mt-10">{track.courseSlugs.map((slug,index)=>{const skill=skillCatalog[slug];if(!skill)return null;return <div key={slug} className="grid grid-cols-[45px_1fr] gap-4 border-t py-7 sm:grid-cols-[45px_1fr_auto]"><span className="font-display text-xl text-zinc-400">0{index+1}</span><div><div className="flex flex-wrap items-center gap-x-3 gap-y-1"><p className="eyebrow">Skill course</p><span className="text-[10px] font-bold uppercase tracking-[.14em] text-cobalt">◷ {skill.duration}</span></div><h3 className="mt-2 font-display text-3xl">{skill.name}</h3><p className="mt-2 text-xs text-zinc-500">{skill.summary}</p></div><a href={`/skill-courses/${slug}`} className="button-secondary mt-4 min-h-10 px-4 text-xs sm:mt-0 sm:self-center">Enroll in Course</a></div>})}<div className="grid grid-cols-[45px_1fr] gap-4 border-b bg-blue-50 p-7"><span className="font-display text-xl text-cobalt">✓</span><div><p className="eyebrow text-cobalt">Projects + certification</p><h3 className="mt-2 font-display text-3xl">Prove you can do the work</h3><p className="mt-2 text-xs text-zinc-500">Complete {track.projectCount} reviewed projects and earn the {track.name} certification.</p></div></div></div></div>
    </div></section>
