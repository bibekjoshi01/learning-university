...
export default function RolePage({params}) {
  ...
  return <main>
    <section className="bg-night text-white"><div className="shell py-20 sm:py-28">
      ...
      <div className="mt-12 grid max-w-3xl grid-cols-2 gap-px overflow-hidden rounded-xl bg-zinc-700 sm:grid-cols-4">{[['Duration',track.duration],['Difficulty',track.difficulty],['Projects',track.projects],['Outcome',track.roles[0]]].map(([key,value])=><div className="bg-night p-5" key={key}><span className="eyebrow !text-zinc-600">{key}</span><strong className="mt-2 block text-sm">{value}</strong></div>)}</div>
    </div></section>

    <section className="section shell" id="roadmap"><div className="grid gap-14 lg:grid-cols-[.65fr_1.35fr]">
      <TrackPreparation roles={track.roles} guidance={guidance}/>
      <div><p className="eyebrow">Learning roadmap</p><h2 className="mt-6 font-display text-5xl sm:text-6xl">Skills build in sequence.</h2><div className="mt-10">{track.skills.map((slug,index)=>{const skill=skillCatalog[slug];return <div key={slug} className="grid grid-cols-[45px_1fr] gap-4 border-t py-7 sm:grid-cols-[45px_1fr_auto]"><span className="font-display text-xl text-zinc-400">0{index+1}</span><div><p className="eyebrow">Skill course</p><h3 className="mt-2 font-display text-3xl">{skill[0]}</h3><p className="mt-2 text-xs text-zinc-500">{skill[1]}</p></div><a href={`/skill-courses/${slug}`} className="button-secondary mt-4 min-h-10 px-4 text-xs sm:mt-0 sm:self-center">Enroll in Course</a></div>})}<div className="grid grid-cols-[45px_1fr] gap-4 border-b bg-blue-50 p-7"><span className="font-display text-xl text-cobalt">✓</span><div><p className="eyebrow text-cobalt">Projects + certification</p><h3 className="mt-2 font-display text-3xl">Prove you can do the work</h3><p className="mt-2 text-xs text-zinc-500">Complete {track.projects} reviewed projects and earn the {track.name} certification.</p></div></div></div></div>
    </div></section>
