...
export default function Header() {
  ...
  return (
    <>
      <div className="bg-ink px-4 py-2.5 text-center text-[11px] font-semibold text-zinc-300">
        Admissions are open for the October cohort. <a className="ml-2 text-white" href="/schools">Explore Tracks →</a>
      </div>
      <header className="sticky top-0 z-50 border-b bg-paper/90 backdrop-blur-xl">
        ...
        {open && <nav className="shell flex flex-col gap-5 border-t py-6 text-sm font-bold lg:hidden">{links.map(([label, href]) => <a onClick={() => setOpen(false)} href={href} key={label}>{label}</a>)}</nav>}
      </header>
    </>
  );
}
