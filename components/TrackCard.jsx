...
export default function TrackCard({ track }) {
  ...
  return <a href={`/roles/${track.slug}`} className="card group flex min-h-[310px] flex-col p-6 transition hover:-translate-y-1 hover:shadow-card sm:min-h-[350px] sm:p-7">
    ...
    <div className="mt-auto border-t pt-5"><div className="flex flex-wrap gap-4 text-[10px] font-bold text-zinc-500"><span>{track.duration}</span><span>·</span><span>{track.difficulty}</span><span>·</span><span>{track.projects} projects</span></div><span className="mt-5 flex items-center justify-between text-xs font-bold text-cobalt">Enroll in Track <ArrowRightIcon className="size-4"/></span></div>
  </a>;
}
